## fit main models with maximum dispersal distance to only range shift observations we are more certain about 
library(tidyverse)
library(ggplot2)
library(dotwhisker)
library(cowplot)
...
## FILTER TO STUDIES THAT ARE NOT TWO TIME POINTS OR STUDIES THAT ARE 2 TIME POINTS BUT AT LEAST 30 YEARS APART
data <- filter(data, Duration >= 30 & Sampling == "TWO" | Sampling != "TWO") 
...
saveRDS(main_models, "data-processed/modelfits_main_allobs_cadillac.rds")
...
table_main <- coefs %>% 
  gt() %>%
  tab_header(
    title = "Cadillac data - all observations"
  ) 

gtsave(table_main, path = "figures/model_results", filename = "table_main-models_all-observations_cadillac.png")
...
ggsave(path = "figures/model_results", filename = "main-model-predictions_all-observations_cadillac.png", 
       width = 12.5, height = 4)
...
saveRDS(di_models, "data-processed/modelfits_main_diobs_cadillac.rds")
...
table_di <- coefs %>% 
  gt() %>%
  tab_header(
    title = "Cadillac data - dispersal-insufficient observations"
  ) 

gtsave(table_di, path = "figures/model_results", filename = "table_main-models_di-observations_cadillac.png")
...
ggsave(path = "figures/model_results", filename = "main-model-predictions_di-observations_cadillac.png", 
       width = 8.5, height = 4)
...
saveRDS(ds_models, "data-processed/modelfits_main_dsobs_cadillac.rds")
...
table_ds <- coefs %>% 
  gt() %>%
  tab_header(
    title = "Cadillac data - dispersal-sufficient observations"
  ) 

gtsave(table_ds, path = "figures/model_results", filename = "table_main-models_ds-observations_cadillac.png")
...
ggsave(path = "figures/model_results", filename = "main-model-predictions_ds-observations_cadillac.png", 
       width = 8.5, height = 4)
