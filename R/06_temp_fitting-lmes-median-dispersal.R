...
round(length(which(data$MedianDispersalPotentialKmY > data$ClimVeloKmY_RelScale))/nrow(data)*100, digits = 0) ## 62 % max disp. mean cv.
round(length(which(data$MedianDispersalPotentialKmY > data$q3ClimVeloKmY_RelScale))/nrow(data)*100, digits = 0) ## 55 % max disp. q3 cv
## birds:
birds = filter(data, group == "Bird")
round(length(which(birds$MedianDispersalPotentialKmY > birds$ClimVeloKmY_RelScale))/nrow(birds)*100, digits = 0) ## 97 % max disp. mean cv.
## plants
plants = filter(data, group == "Plant")
round(length(which(plants$MedianDispersalPotentialKmY > plants$ClimVeloKmY_RelScale))/nrow(plants)*100, digits = 0) ## 29% max disp. mean cv.
## median
...
## show range of climate velocities sampled in birds versus plants 
violin = data %>%
  ggplot(aes(x = group, y = ClimVeloKmY_RelScale, colour = group)) + 
  geom_violin() +
  labs(y = "Rate of isotherm shift (km/y)", x = "Group") +
  theme(legend.position = "none") +
  geom_point(position = position_jitterdodge(), size = .8)

ggsave(path = "figures", filename = "cv-by-group_violin.png", 
       width = 4.2, height = 3.2)

data %>%
  ggplot(aes(x = MedianDispersalPotentialKmY < ClimVeloKmY_RelScale, y = ClimVeloKmY_RelScale)) + 
  geom_violin() +
  labs(y = "Rate of isotherm shift (km/y)") +
  theme(legend.position = "none") +
  geom_point(position = position_jitter())

## make a plot 
data %>%
  mutate(is_faster = ifelse(ClimVeloKmY_RelScale > MedianDispersalPotentialKmY, 
...
disp_plot <- data %>%
  ...
  labs(x = "Potential dispersal rate (km/y)",
       y = "Estimated range expansion rate (km/y)", 
       colour = 'Velocity of\nisotherm\nshift (km/y)') +
...
cv_plot <- data %>%
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.position = "none") +
  scale_y_continuous(limits = c(-6, 26), expand = c(0,0.5)) +
  labs(x = "velocity of isotherm shift (km/y)",
       y = "Estimated range expansion rate (km/y)", 
       colour = 'Velocity of\nisotherm\nshift (km/y)') +
  scale_colour_gradient2(high = "#B2182B", low = "#2166AC", mid = "#F8DCCB", midpoint = 3.5) +
...
table1 = coefs %>%
  ...
  group_by(Model) %>%
  mutate(MeanRank = mean(ModelRank), CumulativeWeight = sum(as.numeric(`AIC weight`))) %>%
  select(Model, MeanRank, CumulativeWeight) %>%
  distinct() %>%
  ungroup() %>%
  rename("Mean model rank" = MeanRank, "Cumulative weight" = CumulativeWeight)
