...
model {
  ...
  for(i in 1:n){
    if (disp[i] < climVelo) {
      ...
      shift[i] ~ gamma((climVelo*b2)^2 / sigma^2, (climVelo*b2) / sigma^2);
    }
  }
...
generated quantities {
  ...
  for(i in 1:n){
    if (disp[i] < climVelo) {
      ...
      shift_pred[i] = gamma_rng((climVelo*b2)^2 / sigma^2, (climVelo*b2) / sigma^2);
    }
  }
  ...
  for (j in 1:n) {
    if (disp[j] < climVelo) {
      ...
      log_lik[j] = gamma_lpdf(shift[j] | (climVelo*b2)^2 / sigma^2, (climVelo*b2) / sigma^2);
    }
