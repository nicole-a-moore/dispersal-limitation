...
length(which(duplicated(classif$SpeciesName))) ## 58 have 2 classifications 

## exclude them for now 
classif <- filter(classif, !SpeciesName %in% classif$SpeciesName[which(duplicated(classif$SpeciesName))])

length(unique(classif$SpeciesName)) ## growth form for 1801 bioshift species for now 
...
length(unique(try_tv$SpeciesName)) ## potential for 998 bioshifts species 
...
length(unique(try_sm$SpeciesName)) ## potential for 3078 bioshifts species 
...
## write
write.csv(all, "data-processed/plant-dispersal-traits.csv", row.names = F)

all <- read.csv("data-processed/plant-dispersal-traits.csv")
...
df_simple %>%
  left_join(., v3, by = c("Species" = "scientificName_checked")) %>%
  filter(Type == "LAT", !Param %in% c("O", "TE") & mean_cv_sppspecific >= 0) %>%
  filter(Rate >= (mean(.$Rate) - sd(.$Rate))) %>%
  mutate(predicted_disprate = ((10^log10MDD)/1000)) %>%
  mutate(limitingrate = ifelse(predicted_disprate < mean_cv_sppspecific, predicted_disprate, mean_cv_sppspecific)) %>%
  ggplot(aes(x = limitingrate,
         y = Rate, colour = limitingrate == predicted_disprate)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
