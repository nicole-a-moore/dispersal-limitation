...
## get rid of squamates and mammals 
dd <- filter(dd, group %in% c("Aves", "Plants"))
...
data %>%
  ...
  scale_fill_gradient2(high = "#B2182B", low = "#2166AC", mid = "#F8DCCB", midpoint = 3.5) +
  labs(x = "Spatial scale of velocity of isotherm shift", y = "Local range expansion rate (km/yr)", 
       shape = "") + 
  guides(size = "legend", fill = "none")
...
legend <- data %>%
  ...
  mutate(cat = "q1, mean, and q3 velocity of isotherm shift") %>%
