...
## save:
ggsave(all_polys, path = "figures", filename = "map_all-polys.png", width = 7.8, height = 4)
...
ggsave(legend_plot, path = "figures", filename = "map_legend.png", 
       width = 4, height = 5)
...
two_sp = ggplot(world_map) + 
  ...
  ## add outlines of clipped study polys 
  geom_sf(data = filter(polys, ID %in% c("Carduelis_carduelis_A129_P1", "Prunus_avium_A156_P1")), colour = "black",
          aes(colour = Species_sdt, fill = Species_sdt), linewidth = 0.2, alpha = 0.5, linetype = "longdash",
          inherit.aes = F) +
  ...
  scale_colour_manual(values = c("#e49e00", 
                                 "#A2B06D")) +
  ## add study area polygon outlines
  geom_sf(data = filter(studies, Name %in% c("A129_P1", "A156_P1")), fill = "transparent", colour = "black", 
          linewidth = 0.2, inherit.aes = F) 

## save:
ggsave(two_sp, path = "figures", filename = "map_inset.png", width = 4, height = 4)
...
ggsave(one, path = "figures", filename = "map_goldfinch.png", width = 3, height = 3)
ggsave(two, path = "figures", filename = "map_redmaple.png", width = 4, height = 3)
...
## write
write.csv(stats, "figures/stats-for-methodological-figure.csv", row.names = F)
