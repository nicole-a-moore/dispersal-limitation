...
## read in data
v1 <- read.csv("data-processed/v1_potential-dispersal-rate.csv")

##################################
##       plot expectations      ##
##################################
hist(v1$ClimVeloTKmY)
...
gradients <- select(ungroup(v1), ClimVeloTKmY, Gradient) %>%
  distinct()
...
## zoom in
filtered_lags <- filter(v1, ClimVeloTKmY < 0.5)
...
lat <- filter(v1, Gradient == "Latitudinal") %>%
