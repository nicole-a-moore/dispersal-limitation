...
disp_plot_di_mean <- di_data_mean %>%
  ggplot(aes(x = DispersalPotentialKmY, y = ShiftKmY, colour = ClimVeloKmY_RelScale)) +
  geom_point(alpha = 0.7, aes(shape = group)) +
  geom_point(data = filter(di_data_mean, is.na(colour), group == "Plants"), colour = "black", inherit.aes = FALSE,
             fill = "transparent", pch = 0,
             aes(x = DispersalPotentialKmY, y = ShiftKmY, shape = group)) +
  geom_point(data = filter(di_data_mean, is.na(colour), group == "Aves"), colour = "black", inherit.aes = FALSE,
             fill = "transparent", pch = 1,
             aes(x = DispersalPotentialKmY, y = ShiftKmY, shape = group)) +
  geom_point(data = filter(di_data_mean, is.na(colour), group == "Squamata"), colour = "black", inherit.aes = FALSE,
             fill = "transparent", pch = 5,
             aes(x = DispersalPotentialKmY, y = ShiftKmY, shape = group)) +
  geom_point(data = filter(di_data_mean, is.na(colour), group == "Mammalia"), colour = "black", inherit.aes = FALSE,
             fill = "transparent", pch = 2,
             aes(x = DispersalPotentialKmY, y = ShiftKmY, shape = group)) +
  theme_bw() +
  stat_function(colour = "grey", fun = function(x){x},
...
#   ggplot(aes(x = DispersalPotentialKmY, y = ShiftKmY, colour = ClimVeloKmY_RelScale)) +
#   geom_point(alpha = 0.7, aes(shape = group)) +
#   geom_point(data = filter(ds_data_mean, is.na(colour), group == "Plants"), colour = "black", inherit.aes = FALSE,
#              fill = "transparent", pch = 0,
#              aes(x = DispersalPotentialKmY, y = ShiftKmY, shape = group)) +
#   geom_point(data = filter(ds_data_mean, is.na(colour), group == "Aves"), colour = "black", inherit.aes = FALSE,
#              fill = "transparent", pch = 1,
#              aes(x = DispersalPotentialKmY, y = ShiftKmY, shape = group)) +
#   geom_point(data = filter(ds_data_mean, is.na(colour), group == "Squamata"), colour = "black", inherit.aes = FALSE,
#              fill = "transparent", pch = 5,
#              aes(x = DispersalPotentialKmY, y = ShiftKmY, shape = group)) +
#   geom_point(data = filter(ds_data_mean, is.na(colour), group == "Mammalia"), colour = "black", inherit.aes = FALSE,
#              fill = "transparent", pch = 2,
#              aes(x = DispersalPotentialKmY, y = ShiftKmY, shape = group)) +
#   theme_bw() +
#   stat_function(colour = "grey", fun = function(x){x},
