...
round(length(which(data$DispersalPotentialKmY > data$ClimVeloKmY_RelScale))/nrow(data)*100, digits = 0) ## 61.99 % max disp. mean cv.
round(length(which(data$DispersalPotentialKmY > data$q3ClimVeloKmY_RelScale))/nrow(data)*100, digits = 0) ## 55.11 % max disp. q3 cv
## birds:
birds = filter(data, group == "Bird")
round(length(which(birds$DispersalPotentialKmY > birds$ClimVeloKmY_RelScale))/nrow(birds)*100, digits = 0) ## 96.62 % max disp. mean cv.
## plants
...
data %>%
  ggplot(aes(x = DispersalPotentialKmY < ClimVeloKmY_RelScale, y = ClimVeloKmY_RelScale)) + 
  geom_violin() +
...
round(length(which(data$DispersalPotentialKmY > abs(data$ShiftKmY)))/nrow(data)*100, digits = 2) ## 56.77
round(length(which(data$MedianDispersalPotentialKmY > abs(data$ShiftKmY)))/nrow(data)*100, digits = 2) ## 47.27
...
cv_plot_di <- cv_plot_di +
  theme(strip.text.x = element_blank(),
        strip.background =  element_blank()) +
  annotate("text", x = 7, y = 23, hjust = 0,
           label = paste0("AICc = ", coefs_saved$AICc[which(coefs_saved$Model == "velocity of isotherm shift")[1]],
                          "\nR2 = ", coefs_saved$R2[which(coefs_saved$Model == "velocity of isotherm shift")[1]]), colour = "black",
           size = 3.5) 
